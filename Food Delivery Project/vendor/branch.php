...
<h4 class="mb-0 float-left">Branches</h4>
...
<?php
$count = "1";
$query = "SELECT * FROM `tbl_branch` WHERE `user_id`='2'";
...
?>
<a href="branch_save.php?Unsuspend_id=<?=$row['id'];?>"  class="btn btn-success btn-sm"><?php echo $row['status'];?></a>
<?php
} elseif ($row['status'] == "Suspend"){
?>
<a href="branch_save.php?Suspend_id=<?=$row['id'];?>"  class="btn btn-danger btn-sm"><?php echo $row['status'];?></a>
<?php
...
?>
    ...
    <td><div class="d-flex">
<a href="javascript:void(0);" class="btn btn-primary shadow btn-xs sharp mr-1" onclick="show('<?=$row['id'];?>', '<?=$row['name']?>', '<?=$row['phone']?>', '<?=$row['address']?>')"><i class="fa fa-pencil"></i></a>
<a href="branch_save.php?del_id=<?php echo $row['id']?>" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>
</div></td>
</tr>
<?php } ?>
</tbody>
...
<h5 class="modal-title">Insert Branches</h5>
<button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span>
</button>
</div>
<form action="branch_save.php" id="addusers" class="needs-validation" method="POST" novalidate>
...
<div class="col-md-6">
<div class="form-group"><label>Branch Contact</label>
...
<script>
function show(id,name,phone,address){
$('#id').val(id);
$('#upd_name').val(name);
$('#upd_phone').val(phone);
