...
<h4 class="mb-0 float-left">Users</h4>
...
<?php
$count = "1";
$query = "SELECT tbl_user.*, roles.role AS rolename FROM `tbl_user`join `roles` ON roles.id=tbl_user.role WHERE tbl_user.role = '1'";
...
<td><input name="chk_id[]" type="checkbox" class='chkbox' value="<?php echo $row['id']; ?>"/></td>
    ...
    <td><a class="fancybox" data-fancybox="gallery" href="backassets/images/users/<?php echo $row['image']?>"><img src="backassets/images/users/<?php echo $row['image']?>" class="img-fluid rounded-circle" alt="" style="width: 40px; height: 40px;"></a></td>
...
?>
<a href="user_save.php?Unsuspend_id=<?=$row['id'];?>"  class="btn btn-success btn-sm"><?php echo $row['status'];?></a>
<?php
} elseif ($row['status'] == "Suspend"){
?>
<a href="user_save.php?Suspend_id=<?=$row['id'];?>"  class="btn btn-danger btn-sm"><?php echo $row['status'];?></a>
<?php
...
<a href="user_save.php?del_id=<?php echo $row['id']?>" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>
</div></td>
</tr>
<?php } ?> 
</tbody>
...
<h5 class="modal-title">Insert Users</h5>
<button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span>
</button>
</div>
<form action="user_save.php" id="addusers" class=" needs-validation" method="POST" novalidate enctype="multipart/form-data">
...
<form action="user_save.php" id="updatepro" method="POST" novalidate enctype="multipart/form-data">
