...
<h4 class="mb-0 float-left">Vendors</h4>
...
<div class="card-body">
<div id="message2"></div>
<form action="vendor_save.php" method="post">
...
<?php
$count = "1";
$query = "SELECT tbl_user.*, roles.role AS rolename FROM `tbl_user`join `roles` ON roles.id=tbl_user.role WHERE tbl_user.role = '2'";
...
<td><input name="chk_id[]" type="checkbox" class='chkbox' value="<?php echo $row['id']; ?>"/></td>
    ...
    <td><a class="fancybox" data-fancybox="gallery" href="../vendor/backassets/images/vendors/<?php echo $row['image']?>"><img src="../vendor/backassets/images/vendors/<?php echo $row['image']?>" class="img-fluid rounded-circle" alt="" style="width: 40px; height: 40px;"></a></td>
...
?>
<a href="vendor_save.php?Unsuspend_id=<?=$row['id'];?>"  class="btn btn-success btn-sm"><?php echo $row['status'];?></a>
<?php
} elseif ($row['status'] == "Suspend"){
?>
<a href="vendor_save.php?Suspend_id=<?=$row['id'];?>"  class="btn btn-danger btn-sm"><?php echo $row['status'];?></a>
<?php
...
<a href="vendor_save.php?del_id=<?php echo $row['id']?>" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a>
</div></td>
</tr>
<?php } ?>
</tbody>
...
<h5 class="modal-title">Insert Vendors</h5>
<button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span>
</button>
</div>
<form action="vendor_save.php" id="addusers" class="needs-validation" method="POST" novalidate enctype="multipart/form-data">
...
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title">Update Vendors</h5>
<button type="button" class="close" data-dismiss="modal" aria-label="Close" id="upd_close"> <span aria-hidden="true">&times;</span>
</button>
</div>
<form action="vendor_save.php" id="updatepro" method="POST" novalidate enctype="multipart/form-data">
...
<select name="upd_role" id="upd_role" class="form-control" >
<?php
...
<i class="fa fa-eye" aria-hidden="true" style="margin-top: 43px;
    position: absolute;
    margin-left: 93px;" onclick="gggg()"></i>
